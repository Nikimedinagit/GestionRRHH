...
$(document).ready(function () {
    $("#EmpleadoIdBuscar, #NroLegajoFiltro, #FechaInicioBuscar, #FechaFinBuscar")
        .on("input change", function () {
            ObtenerEvaluacionesPorEmpleado();
        });

});

// =========================== Obtener Listado desde la API ====================================
async function ObtenerEvaluacionesPorEmpleado() {
    const filtro = {
    ...
    try {
        const response = await authFetch("Resultados/EmpleadoEvaluacionesN2", {
            method: "POST",
            body: JSON.stringify(filtro)
        });

        const data = await response.json();
        MostrarEvaluacionesPorEmpleado(data);
...
mqTabletMobile.addEventListener("change", () => {
    if (window._cacheEvaluacionesEmpleado)
        MostrarEvaluacionesPorEmpleado(window._cacheEvaluacionesEmpleado);
});
...
function MostrarEvaluacionesPorEmpleado(data) {
    window._cacheEvaluacionesEmpleado = data;
    const tabla = $("#listadoEvalaucionesPorEmpelado");
    tabla.empty();
...
async function GenerarInformePdfListadoEvaluacionEmpleado() {
    const empleados = await ObtenerEvaluacionesPorEmpleado();
    ...
    doc.text("Informe de Evaluaciones por Empleado", doc.internal.pageSize.getWidth() / 2, 20, { align: "center" });
    ...
    w.document.title = "Informe de Evaluaciones por Empleado";
    w.document.close();
}
